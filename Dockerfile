...
# Argo CD UI stage
####################################################################################################
FROM --platform=$BUILDPLATFORM node:12.18.4 as argocd-ui
...
####################################################################################################
FROM --platform=$BUILDPLATFORM golang:1.16.2 as argocd-build
...
RUN GOOS=$(echo $TARGETPLATFORM | cut -f1 -d/) && \
    ...
    make GOOS=${GOOS} GOARCH=${GOARCH} GOARM=${GOARM} argocd-all
